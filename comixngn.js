...
5,a,b,d+1)};window.setTimeout(e,5,d,Math.floor(b/5),0);return d},h=function(){f[this.imaginaryID].loaded=!0},p=function(a,b){x=!0;window.setTimeout(E,m.rate,D);A();f[b].loaded||z.clearRect(0,0,e[1].width,e[1].height);a.imaginaryID=b;a.src=n.dir+f[b].url[0];for(var d=0,c=b-1;c>b-n.imgprebuffer-1&&0<=c;c--)f[c].loaded||(t[d].imaginaryID=c,t[d].src=n.dir+f[c].url,d++);d=0;for(c=b+1;c<n.imgpostbuffer+b+1&&c<l;c++)f[c].loaded||(r[d].imaginaryID=c,r[d].src=n.dir+f[c].url,d++)};this.count=function(){return l};
this.current=function(){return q};this.callback=function(a,b){if(null===a||void 0===a)return u;if(null===b||void 0===b)return a?0<a?A:B:u;a?0<a?A=b:B=b:u=b;return 1};this.go=function(a){a=null===a||void 0===a?0:parseInt(a,10);a=isNaN(a)?0:a;p(g,Math.floor(Math.max(0,Math.min(l-1,a))));return a};this.prev=function(){var a=q-1;0<=a&&p(g,a);return a};this.next=function(){var a=q+1;a<l&&p(g,a);return a};this.frst=function(){0<=q&&p(g,0);return 0};this.last=function(){p(g,l-1);return l-1};this.rand=function(){var a=
Math.floor(Math.random()*(l-1));p(g,a);return a};this.data=function(a){a=null===a||void 0===a?0:parseInt(a,10);return isNaN(a)?f[q]:f[a]};this.scroll=function(a){return null===a||void 0===a?y:y=a};this.scrollTo=function(a,b){return F(a,b)};e[0].height=480;e[0].style.background=m.back;e[0].style.paddingLeft="170px";e[0].style.zIndex=0;e[0].style.position="absolute";k?k.appendChild(e[0]):document.body.appendChild(e[0]);window.setTimeout(E,m.rate,D);g=new Image;g.imaginaryID=-1;g.addEventListener("load",
function(){f[this.imaginaryID].loaded?z.clearRect(0,0,this.width,this.height):f[this.imaginaryID].loaded=!0;u();e[1].width=this.width;e[1].height=e[0].height=this.height;z.drawImage(this,0,0);q=this.imaginaryID;x=0;y&&F();B()},!1);for(b=0;b<f.length;b++)f[b].desig=b?b==f.length-1?1:0:-1,f[b].loaded=!1;for(b=0;b<c.config.imgprebuffer;b++)t.push(new Image),t[b].imaginaryID=-1,t[b].addEventListener("load",h,!1);for(b=0;b<c.config.imgpostbuffer;b++)r.push(new Image),r[b].imaginaryID=-1,r[b].addEventListener("load",
h,!1);p(g,void 0===v||null===v?n.startpage:v);e[1].height=480;e[1].width=640;e[1].background=n.back;e[1].style.zIndex=1;e[1].style.position="relative";k?k.appendChild(e[1]):document.body.appendChild(e[1])};
    /**/
