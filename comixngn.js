...
cG.REPO.stage = { "def": { id: "def", construct: function (name, scriptt, anchor, options) {
            ...
            var direction = function (input, anchor, owrite, c, mode) {
                ...
                if (void 0 === input) {
                    ...
                    for (var q = 0; q < input.length; q++) {
                        ...
                        if (Array.isArray(input[q])) {
                            ...
                            holdr.pages[q].url.push(input[q]);
                    }
                    input = holdr;
                }
                    ...
                    input.pages.push({ alt: "",
                        ...
                        perm: !1,
                        anim8: !1 });
                else if (void 0 === input.pages[0].url)
                    return -1;
                if (void 0 === anchor || anchor == null)
                    anchor = 0;
                //PROPERTIES - private
                var iimg = input.pages, count = input.pages.length, spinning = true, //is the spinner spinning?
                current = -1, //-1 for unset, corresponds to current page,
                spinner = input.loading, config = input.config, parent = input.parent, offset = input.offset, pstload = [], preload = [], master = new Image(), skroll = true, skrlln = false, layers = [document.createElement("canvas"), document.createElement("canvas")], context = layers[1].getContext('2d'), 
                //METHODS - private
                n = function () { return 0; }, //this null fuction save us some bytes
                cb = {
                    run: function (a) { for (var b = 0; b < cb[a].length; b++) {
                        cb[a][b]();
                    } },
                    start: [],
                    slidn: [],
                ...
                object = {
                    ...
                    if (to === null || void 0 === to)
                        ...
                        to = { x: 0, y: to }; //if to is num assume its y
                    else {
                    ...
                    if (to.y < 0)
                        to.y = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
                    if (to.x < 0)
                        to.x = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
                    ...
                    var clock = function (c, b, a) {
                        if (skrlln) {
                            ...
                            if (a + 1 < b * 5)
                                window.setTimeout(clock, 5, c, b, a + 1);
                            else
                                deactive();
                        }
                    };
                ...
                this.src="";*/
                    ...
                    if (idd >= count)
                        idd = count - 1; //can not be equal to our higher than the amount of pages
                    if (!iimg[idd].loaded)
            ...
            if(c.jq) jq();*/
                ...
                this.rand = function () {
                    ...
                    if (sre == current)
                        sre += Math.floor(Math.random() * (count - 2)) + 1;
                    //console.log(sre);
                    assign(master, sre);
                    return sre;
                };
...
cG.HELPERS.smartAttrib = function (source, mapper, ignore) {
    ...
    if (void 0 !== srch && ig <= 0) {
        if (srch.count === void 0 || srch.count != 0) { /*as long as count != 0 we can set the attribute*/
            ...
            if (srch.count > 0)
                mapper[source.nodeType.toLowerCase()].count--; /*if count is above 0, decrement it (this limits the amount of sets)*/
        }
    }
    else
        ig--;
...
cG.controlInjection = function (SPECIFIC) {
    ...
    var stages = [], ctrls = (cG.ctrls) ? cG.ctrls : '<ul><li style="display: inline;"><button class="frst" >|&lt;</button></li><li style="display: inline;"><button class="prev" rel="prev" accesskey="p">&lt; Prev</button></li><li style="display: inline;"><button class="rand" >Random</button></li><li style="display: inline;"><button class="next" rel="next" accesskey="n">Next &gt;</button></li><li style="display: inline;"><button class="last" >&gt;|</button></li></ul>', antictrl = '<ul><li style="display: inline;"><button class="last" >&lt;|</button></li><li style="display: inline;"><button class="next" rel="next" accesskey="n">Next &lt;</button></li><li style="display: inline;"><button class="rand" >Random</button></li><li style="display: inline;"><button class="prev" rel="prev" accesskey="p">&gt; Prev</button></li><li style="display: inline;"><button class="frst" >|&gt;</button></li></ul>', pod, podling, errr = "controlInjection can only operate on elements or arrays of elements", eventer = function (par, chd) {
    ...
    var exist = document.querySelectorAll('[cglink]'), linkcg;
    //console.log(exist)
